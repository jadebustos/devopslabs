...
```console
[kubeadmin@kubemaster ~]$ kubectl apply -f https://raw.githubusercontent.com/kubernetes/dashboard/v2.0.0/aio/deploy/recommended.yaml
...
```console
[kubeadmin@kubemaster ~]$ kubectl get svc -A
...
[kubeadmin@kubemaster ~]$ kubectl edit service kubernetes-dashboard --namespace=kubernetes-dashboard
```
...
```console
[kubeadmin@kubemaster ~]$ kubectl -n kubernetes-dashboard get secret $(kubectl -n kubernetes-dashboard get sa/admin-user -o jsonpath="{.secrets[0].name}") -o go-template="{{.data.token | base64decode}}"
...
```console
[kubeadmin@kubemaster ~]$ kubectl get svc --namespace=kubernetes-dashboard
